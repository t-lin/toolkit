...
int main( int argc, char* argv[] )
{
    ...
    // Loop through all the address candidates and connect to the first one we can
    int connSocket = 0;
    struct addrinfo* pTmpAddr = NULL;
    for( pTmpAddr = pServAddrList; pTmpAddr != NULL; pTmpAddr = pTmpAddr->ai_next )
    {
        // Create socket
        connSocket = socket( pTmpAddr->ai_family, pTmpAddr->ai_socktype, pTmpAddr->ai_protocol );
        if( connSocket == -1 )
        {
        ...
        if( connect( connSocket, pTmpAddr->ai_addr, pTmpAddr->ai_addrlen ) == -1 )
        {
    ...
    while( gINTERRUPTED == false )
    {
        ...
        ssize_t numbytes = recv( connSocket, buf, (size_t)( MAXDATASIZE - 1 ), 0 );
        if( numbytes == -1 )
        {
            perror( "recv" );
            exit( 1 );
