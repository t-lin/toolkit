...
class CircularBufferIterator {
  public:
    ...
    typedef ptrdiff_t difference_type;  // TODO: Unsure about this...
    typedef T value_type;
    typedef T* pointer;
    typedef T& reference;
  ...
  public:
    ...
    CircularBufferIterator& operator++() noexcept {
      //fprintf(stderr, "TLIN DEBUG: prefix ++ called\n");
      if (pCircBuf_->size_ != 0 && arrCurrIdx_ != END) {
        // TODO: check tail doesn't change?
    ...
    CircularBufferIterator operator++(int) noexcept {
      //fprintf(stderr, "TLIN DEBUG: postfix ++ called\n");
    ...
    CircularBufferIterator& operator--() {
      ...
      if (pCircBuf_->size_ != 0 && arrCurrIdx_ != pCircBuf_->head_) {
        // TODO: check head doesn't change?
    ...
    CircularBufferIterator& operator+=(const difference_type diff) {
      // TODO: This will enable us to reclassify as random access iterator type
      if (diff < 0) {
      ...
      if (udiff > pCircBuf_->size_) {
        arrCurrIdx_ = END;
      } else {
        // Calculate distance between the current index and the tail.
        ...
        if (pCircBuf_->tail_ >= arrCurrIdx_) {
          ...
          distToEnd = N - static_cast<uint64_t>(delta);
        }
    ...
    CircularBufferIterator& operator-=(const difference_type n) {
      if (n < 0) {
        return operator+=(-n);
      }
