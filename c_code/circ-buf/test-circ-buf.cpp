...
CircularBuffer<uint8_t, TARGET_SIZE> createBufferNoRotatation(uint8_t sz) {
  // Should not add more than the capacity.
  if (sz > TARGET_SIZE) {
    throw std::invalid_argument("sz cannot be greater than TARGET_SIZE\n");
  }
...
CircularBuffer<uint8_t, TARGET_SIZE> createRotatedBuffer(uint8_t nRotate) {
  ...
  if (nRotate >= TARGET_SIZE) {
    throw std::invalid_argument("nRotate should be less than TARGET_SIZE\n");
  }

  CircularBuffer<uint8_t, TARGET_SIZE> circBuf = createBufferNoRotatation(TARGET_SIZE);
...
void forwardIteratorTests(CircularBuffer<uint8_t, TARGET_SIZE>& circBuf) {
  ...
  for (auto it = circBuf.end(); it != circBuf.begin();) {
    // Decrement 'it' the first time; dereferencing end() is undefined.
    // TODO: Change initializer to end() - 1 once we support random access
    //       iterators (i.e. +/- n).
    if (it == circBuf.end()) {
      it--;
    }
...
void reverseIteratorTests(CircularBuffer<uint8_t, TARGET_SIZE>& circBuf) {
  ...
  for (auto it = circBuf.rend(); it != circBuf.rbegin();) {
    // Decrement 'it' the first time; dereferencing rend() is undefined.
    // TODO: Change initializer to end() - 1 once we support random access
    //       iterators (i.e. +/- n).
    if (it == circBuf.rend()) {
      it--;
...
TEST(CircularBuffer, Assignment) {
  CircularBuffer<uint8_t, TARGET_SIZE> buf1 = createBufferNoRotatation(TARGET_SIZE);
  CircularBuffer<uint8_t, TARGET_SIZE> buf2 = createRotatedBuffer(5);
  ...
  // Assignment test
  buf1 = buf2;
  buf2 = createBufferNoRotatation(TARGET_SIZE);
  ASSERT_EQ(buf1[0], 6);
}
...
TEST(CircularBufferIterator, IteratorDerefOp) {
  ...
  // Populate buffer.
  circBuf = createBufferNoRotatation(TARGET_SIZE);
...
TEST(CircularBufferIterator, ForwardIteratorWithRotation) {
  ...
  CircularBuffer<uint8_t, TARGET_SIZE> circBuf = createRotatedBuffer(nRotate);
