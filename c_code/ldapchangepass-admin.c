...
int main(int argc, char **argv)
{
    ...
    if (argc != 3) {
        ...
        printf("USAGE: ldapasswd-admin <username> <newpassword>\n");
        exit(EXIT_FAILURE);
    }
    ...
    char ldapIP[4] = {10, 20, 30, 254}; // Set LDAP IP here
    ...
    char *commandToken = "ldappasswd -H ldap://%hhu.%hhu.%hhu.%hhu -x -D \"cn=admin,dc=savitestbed,dc=ca\" -w %s -s %s \"uid=%s,ou=People,dc=savitestbed,dc=ca\"";
    char completedCommand[250] = {0};
    sprintf(completedCommand, commandToken, ldapIP[0], ldapIP[1], ldapIP[2], ldapIP[3], ldapPass, newpass, user);
    //printf("Command: %s\n", completedCommand);
